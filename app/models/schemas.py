...
# ── Analyze (Risk / Earnings / Peers) ─────────────────────────────


class RiskSubScores(BaseModel):
    """Sub-scores of the risk engine (each 0-100, null if missing data)."""
...
class RiskMetric(BaseModel):
    """Raw metric backing a sub-score."""
    value: Optional[float] = None
    display_value: str = "N/A"


class RiskAnalysis(BaseModel):
    """Risk analysis response: total 0-100 + sub-scores + raw metrics."""
    symbol: str
    total_score: Optional[float] = None
    sub_scores: RiskSubScores = RiskSubScores()
    roe: RiskMetric = RiskMetric()
    net_margin: RiskMetric = RiskMetric()
    pe_ratio: RiskMetric = RiskMetric()
    sector: Optional[str] = None
    sector_pe_benchmark: Optional[float] = None
    flags: list[str] = []
    source: str = "yfinance"
    cached_at: Optional[str] = None
...
class PeersAnalysis(BaseModel):
    """Peer comparison response: full list + top 3 recommendations."""
    symbol: str
    sector: Optional[str] = None
    peers: list[PeerEntry] = []
    top_alternatives: list[PeerEntry] = []
    flags: list[str] = []
    source: str = "yfinance"
    cached_at: Optional[str] = None

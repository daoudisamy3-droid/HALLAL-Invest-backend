...
@router.get(
    ...
    summary="Gemini Strategic Diagnostic",
    description=(
        "Revenue mix, competitors, moat pillars, and SWOT analysis "
        "generated by Gemini from the company description. "
        "Results cached 7 days. Returns clean empty data if Gemini fails."
    ),
    dependencies=[Depends(rate_limit_dependency)],
)
async def get_strategy(symbol: str) -> dict:
    ...
    try:
        result = await get_company_strategy(symbol)
    except Exception as exc:

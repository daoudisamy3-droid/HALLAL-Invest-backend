...
# ── 7-day cache for Gemini strategy ──────────────────────────────
...
@router.get(
    ...
    summary="Strategic Diagnostic (yfinance + Gemini enrichment)",
    description=(
        "Primary data from yfinance: pitch, sector, financial ratios, sentiment. "
        "Optionally enriched by Gemini (revenue_mix, competitors, moat, SWOT). "
        "Cached 7 days. Always returns data even if Gemini fails."
    ),
    dependencies=[Depends(rate_limit_dependency)],
)
async def get_strategy(symbol: str) -> dict:
    ...
    # Check 7-day cache
    now = time.time()
    if symbol in _strategy_cache:
        cached_at, cached_result = _strategy_cache[symbol]
        if now - cached_at < _STRATEGY_CACHE_TTL:
            logger.info("Strategy cache hit for %s", symbol)
            return cached_result

    from app.services.ticker_service import get_strategy_data
    from app.services.business_intelligence import get_company_strategy

    # ── 1. yfinance data (always available) ───────────────────────
    try:
        yf_data = await get_strategy_data(symbol)
    except Exception as exc:
        logger.error("Strategy yfinance extraction failed for %s: %s", symbol, exc)
        raise HTTPException(
            status_code=502,
            detail=f"Could not fetch data for '{symbol}': {exc}",
        )

    # ── 2. Gemini enrichment (best-effort) ────────────────────────
    gemini_data: dict = {}
    try:
        gemini_data = await get_company_strategy(symbol)
    except Exception as exc:
        logger.warning("Gemini enrichment failed for %s: %s — serving yfinance only", symbol, exc)

    # ── 3. Merge: yfinance base + Gemini overlay ──────────────────
    result = {
        **yf_data,
        "revenue_mix": gemini_data.get("revenue_mix", []),
        "competitors": gemini_data.get("competitors", []) or yf_data.get("competitors", []),
        "moat": gemini_data.get("moat", []),
        "swot": gemini_data.get("swot", {"strengths": [], "weaknesses": []}),
        "source": f"yfinance+gemini" if gemini_data.get("source") == "gemini" else "yfinance",
    }

    _strategy_cache[symbol] = (now, result)
    logger.info("Strategy cached for %s (TTL=7d, source=%s)", symbol, result["source"])
